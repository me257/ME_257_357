function [T4,mDot4] = RQLCombustor(phi,UInf, altitude, engine, fluid)
    ...
    f1 = fuelAirRatio(fluid);
    f = f1/(1.0+betaC);
    mDotC = (1+f)*mDotC;
    
    %combust the mixture in the primary reaction zone
    equilibrate(fluid.gas,'HP');
    
    %quench the production of NO and mix the result
    YMix = massFractions(fluid.gas);
    YMix(speciesIndex(fluid.gas,'no'))=0.0;
    YMix = YMix/sum(YMix); %renormalize
    %mix the mass fractions and enthalpy adiabtically
    YMix = (YMix + betaC*YC2)/(1.0+betaC); 
    hMix = (enthalpy_mass(fluid.gas)+betaC*hC2)/(1.0+betaC) ;
    set(fluid.gas,'H',hMix,'P',p3,'Y',YMix)
    
    %equilibrate in secondary reaction zone
    equilibrate(fluid.gas,'HP');
